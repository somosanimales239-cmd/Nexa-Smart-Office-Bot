...
function renderDashboard() {
  ...
  return sectionHeader('Your business at a glance', 'Local data, connected business activity and suggested priorities.', '<button class="primary-button" data-nexa-action="task-create">+ New task</button>') +
    ...
    '<div class="grid dashboard-grid">' +
      ...
      '<article class="panel-card"><div class="panel-header"><div><h3>Smart assistant</h3><p>Get an action plan from your configured provider</p></div>' + (providerConfigured ? badge('Ready', 'success') : badge('Not configured', 'warning')) + '</div><p class="muted">Nexa can review pending work and prepare next actions. Existing customer records remain read-only; an authorized appointment may create one new website Lead through the appointment endpoint.</p><button class="primary-button" data-go="ai" data-nexa-action="navigate-ai">Generate suggestions</button></article>' +
...
function renderKnowledgeCenter() {
  ...
  const appointmentLibrary='<article class="panel-card"><div class="panel-header"><div><h3>Professional Appointment Communication</h3><p>A bilingual state-aware library keeps the corrected date, selected time and contact step locked to the same verified Agenda slot before creating the website Lead.</p></div>'+badge('English + Spanish','success')+'</div><div class="knowledge-summary-grid"><article><span>Appointment phrases</span><strong>371</strong><small>Natural requests, date corrections, selections, phone details and short follow-ups</small></article><article><span>Communication intents</span><strong>40</strong><small>Scheduling, corrections, same-day options, selections, contact details and changes</small></article><article><span>Professional templates</span><strong>90</strong><small>Dynamic replies filled with the active date, selected time, Agenda and contact requirements</small></article><article><span>Closing strategy</span><strong>Lead + Agenda</strong><small>Revalidates one exact slot, creates the website Lead and then confirms the reservation</small></article></div></article>';
...
function dealerOfficeShortcuts() {
  ...
  const reserveAppointment = agenda + '?reserve=1#agenda-reserve-appointment';
  return '<div class="button-row dealer-office-shortcuts">' +
    ...
    '<a class="secondary-button" href="' + esc(reserveAppointment) + '" target="_blank" rel="noreferrer">Reserve Appointment</a>' +
...
function renderAIControl() {
  ...
  return sectionHeader('AI Control', 'Give Nexa limited autonomy only inside parameters you explicitly authorize. Existing records remain read-only; authorized appointment Leads can be created and automatic deletion is impossible.', '<div class="button-row">' + activeLabel + timerLabel + connectedLabel + '</div>') +
    '<form id="automation-form" class="automation-layout" data-nexa-action="automation-save">' +
      ...
      '<article class="panel-card"><div class="panel-header"><div><p class="eyebrow">AUTOMATIC APPOINTMENTS</p><h3>Dealer Appointment Availability</h3><p>Nexa explains verified daily hours, offers available times for that day, proposes the next available day when needed, and creates a calendar appointment only after the customer clearly selects a verified slot. A cordial decline includes dealer contact information.</p></div>' + badge(String(availability.length) + ' slots loaded', availability.length ? 'success' : 'warning') + '</div>' +
        '<div class="automation-grid">' +
          ...
          '<label>Create Lead + appointment on website<select name="auto_appointments_create_remote">' + automationOption('0','Local calendar only',settings.auto_appointments_create_remote) + automationOption('1','Website Lead + reserved Agenda slot',settings.auto_appointments_create_remote) + '</select></label>' +
...
function renderAbout() {
  ...
  return sectionHeader('About', 'A local-first business assistant with user-controlled guarded autonomy.') +
    '<div class="grid split-grid">' +
      ...
      '<article class="panel-card"><p class="eyebrow">AI CONTROL</p><h3>Guarded automatic actions · ' + (active ? 'Enabled' : 'Disabled') + '</h3><p class="muted">With explicit authorization, Nexa may send messages, revalidate a selected slot and create one appointment Lead through the website endpoint. It never edits existing customer records and has no automatic delete capability.</p><div class="button-row">' + badge(String(autoSettings.auto_messages_enabled || '0') === '1' ? 'Messages authorized' : 'Messages manual', String(autoSettings.auto_messages_enabled || '0') === '1' ? 'success' : 'warning') + badge(String(autoSettings.auto_appointments_enabled || '0') === '1' ? 'Appointments authorized' : 'Appointments manual', String(autoSettings.auto_appointments_enabled || '0') === '1' ? 'success' : 'warning') + '</div></article>' +
      '<article class="panel-card"><p class="eyebrow">SAFETY BOUNDARIES</p><h3>Human-owned business records</h3><p class="muted">Existing contacts, leads, orders, reseller records and customer profiles remain read-only to automation. Only a newly confirmed appointment may create its own Lead. Sensitive, legal, financial and dispute messages are escalated for human review.</p></article>' +
  ...
  if (automationForm) {
    automationForm.addEventListener('submit', async function saveAutomation(event) {
      ...
      if (String(data.auto_actions_enabled || '0') === '1') {
        const confirmed = await confirmAction('Authorize guarded automatic actions', 'Nexa will be permitted to send messages and/or create appointment Leads only according to the parameters shown. Existing customer records remain read-only and no automatic delete operation exists.');
        if (!confirmed) return;
      }
      try {
